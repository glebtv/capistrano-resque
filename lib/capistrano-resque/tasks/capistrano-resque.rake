      ...
      on roles(role) do
        ...
        workers.each_pair do |queue, number_of_workers|
          ...
          number_of_workers.times do
            ...
            within current_path do
              SSHKit.config.command_map.prefix[:rake].unshift(%{RAILS_ENV=#{fetch(:rails_env)} QUEUE="#{queue}" PIDFILE=#{pid} BACKGROUND=yes VERBOSE=1 INTERVAL=#{fetch(:interval)}})
              execute :rake, %{#{"environment" if fetch(:resque_environment_task)} resque:work #{output_redirection}}
